...
public class AbstractParserTestFixture {
  ...
  public void SetUp() {
    ...
    ((StringWriter)_compiler.Parameters.OutputWriter)
        .GetStringBuilder()
        .Length = 0;
  }

  protected virtual string GetRelativeTestCasesPath() { return "parser"; }

  protected string GetTestCasePath(string fname) {
    return Path.Combine(BooCompiler.Tests.BooTestCaseUtil.GetTestCasePath(
                            GetRelativeTestCasesPath()),
                        fname);
  }
  ...
  protected virtual Boo.Lang.Compiler.ICompilerInput
  GetCompilerInput(string testfile) {
  ...
  protected string GetDocumentation(Boo.Lang.Compiler.Ast.Module module) {
    ...
    if (null == module.Documentation) {
      Assert.Fail(string.Format("Test case '{0}' does not have a docstring!",
                                module.LexicalInfo.FileName));
    }
    return doc;
  }
