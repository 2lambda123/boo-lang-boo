...
public class BooCodeBuilderTest : AbstractTypeSystemTest {
  ...
  public void GenericTypeReference() {
    RunInCompilerContextEnvironment(() => {
      var genericType =
          TypeSystemServices.Map(typeof(System.Collections.Generic.List<int>));
      ...
      Assert.AreSame(TypeSystemServices.IntType,
                     genericTypeRef.GenericArguments[0].Entity);
  ...
  [Test]
  public void
  CreateMethodFromPrototypeRemapsGenericMethodParametersAndReturnType() {
    RunInCompilerContextEnvironment(() => {
      var result = CodeBuilder.CreateMethodFromPrototype(
          GetMethod("GenericMethodPrototype"), TypeMemberModifiers.Override);
      ...
      Assert.AreSame(genericParameterType.MakeArrayType(1),
                     parameters[0].Type.Entity);
    });
  }
