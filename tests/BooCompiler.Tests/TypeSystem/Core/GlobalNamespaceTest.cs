...
public class GlobalNamespaceTest : AbstractTypeSystemTest {
  ...
  public override void SetUp() {
    ...
    RunInCompilerContextEnvironment(() => _subject =
                                        My<GlobalNamespace>.Instance);
  }
  ...
  [Test]
  public void ParentNamespace() { Assert.IsNull(_subject.ParentNamespace); }

  [Test]
  public void ResolveTopLevelNamespace() {
    RunInCompilerContextEnvironment(delegate {
      var booCompiler =
          (INamespace)NamespaceAssert.ResolveSingle(_subject, "Boo");
  ...
  public void SingleEnumType() {
    RunInCompilerContextEnvironment(
        () => AssertSingleTypeResolution(
            typeof(Boo.Lang.Compiler.Ast.TypeMemberModifiers)));
  }
  ...
  public void InternalTypeWithSameNameAsReferencedType() {
    RunInCompilerContextEnvironment(delegate {
      ...
      IType internalType =
          DefineInternalClass(subjectType.Namespace, subjectType.Name);
      Set<IEntity> found = ResolveQualifiedName(subjectType.FullName);
      Assert.IsTrue(
          found.ContainsAll(new[] { Map(subjectType), internalType }));
    });
  }
  ...
  private IEntity ResolveQualifiedNameToSingle(string qualifiedName) {
    return NamespaceAssert.ResolveQualifiedNameToSingle(_subject,
                                                        qualifiedName);
  }
}
}