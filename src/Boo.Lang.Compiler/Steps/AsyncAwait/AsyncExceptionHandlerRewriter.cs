...
/// <summary>
/// The purpose of this rewriter is to replace await-containing catch and
/// finally handlers with surrogate replacements that keep actual handler code
/// in regular code blocks. That allows these constructs to be further lowered
/// at the async lowering pass.
///
/// Adapted from Microsoft.CodeAnalysis.CSharp.AsyncExceptionHandlerRewriter in
/// the Roslyn codebase
/// </summary>
public sealed class AsyncExceptionHandlerRewriter : DepthFirstTransformer {
  ...
  static AsyncExceptionHandlerRewriter() {
    ...
    var exceptionDispatchInfo =
        tss.Map(typeof(System.Runtime.ExceptionServices.ExceptionDispatchInfo));
    var methods =
        exceptionDispatchInfo.GetMembers().OfType<IMethod>().ToArray();
    _exceptionDispatchInfoCapture =
        methods.SingleOrDefault(m => m.Name.Equals("Capture"));
    _exceptionDispatchInfoThrow = methods.SingleOrDefault(
        m => m.Name.Equals("Throw") && m.GetParameters().Length == 0);
  }

  private AsyncExceptionHandlerRewriter(Method containingMethod,
  ...
  ///     The general strategy is to rewrite await containing handlers into
  ///     synthetic handlers. Synthetic handlers are not handlers in IL sense so
  ///     it is ok to have awaits in them. Since synthetic handlers are just
  ///     blocks, we have to deal with pending exception/branch/return manually
  ...
  ///
  ///     try:
  ///         code  // any gotos/returns are rewritten to code that pends the
  ///         necessary info and goes to finallyLabel goto finallyLabel
  ...
  ///     try:
  ///         code
  ///     except temp as Exception:  // essentially pend the currently active
  ///     exception
  ...
  public static void Rewrite(Method containingMethod) {
    ...
    if (analysis.ContainsAwaitInHandlers()) {
      ...
      var rewriter = new AsyncExceptionHandlerRewriter(containingMethod,
                                                       factory, analysis);
  ...
  public override void OnTryStatement(TryStatement node) {
    ...
    if (!finallyContainsAwaits) {
      ...
      ReplaceCurrentNode(new TryStatement(
          finalizedRegion.LexicalInfo) { EnsureBlock = rewrittenFinally });
      return;
    }
    ...
    var pendingExceptionLocal =
        _F.DeclareTempLocal(_containingMethod, exceptionType);
    var finallyLabel = _F.CreateLabel(
        node.EnsureBlock, context.GetUniqueName("finallyLabel"), _tryDepth);
    var pendingBranchVar = _F.DeclareTempLocal(_containingMethod, _tss.IntType);

    var catchAll = new ExceptionHandler {
      Declaration = new Declaration(
          pendingExceptionLocal.Name,
          _F.CreateTypeReference(exceptionType)) { Entity =
                                                       pendingExceptionLocal },
      Block = new Block(), IsSynthetic = true
    };

    var tryBlock =
        new Block(finalizedRegion, _F.CreateGoto(finallyLabel, _tryDepth),
    ...
    var syntheticFinally = new Block(
        finallyLabel.LabelStatement, rewrittenFinally,
        UnpendException(pendingExceptionLocal),
        UnpendBranches(frame, pendingBranchVar, pendingExceptionLocal));
    ...
    statements.Add(_F.CreateAssignment(
        ...
        _F.CreateDefaultInvocation(LexicalInfo.Empty,
  ...
  private Block PendBranches(AwaitFinallyFrame frame,
    ...
    if (returnProxy != null) {
      PendBranch(bodyStatements, returnProxy, i, pendingBranchVar,
                 finallyLabel);
    }
  ...
  private void PendBranch(Block bodyStatements, InternalLabel proxy, int i,
                          InternalLocal pendingBranchVar,
                          InternalLabel finallyLabel) {
    // branch lands here
    ...
    bodyStatements.Add(_F.CreateAssignment(
        _F.CreateLocalReference(pendingBranchVar), _F.CreateIntegerLiteral(i)));

    // skip other proxies
    bodyStatements.Add(_F.CreateGoto(finallyLabel, _tryDepth));
  }

  private Statement UnpendBranches(AwaitFinallyFrame frame,
                                   InternalLocal pendingBranchVar,
    ...
    if (frame.returnProxyLabel != null) {
      ...
      var returnLabel = parent.ProxyReturnIfNeeded(
          _containingMethod, pendingValue, out returnValue);

      if (returnLabel == null) {
        unpendReturn = new ReturnStatement(
        ...
        if (pendingValue == null) {
          ...
          unpendReturn = new Block(
              new ExpressionStatement(_F.CreateAssignment(
    ...
    var defaultLabel = _F.CreateLabel(
        _containingMethod, CompilerContext.Current.GetUniqueName("default"),
        _tryDepth);
    cases.Insert(0, _F.CreateGoto(defaultLabel, _tryDepth));
    return CreateSwitch(cases, defaultLabel, pendingBranchVar);
  }

  private Block SwitchBlock(Statement body, int ordinal,
                            InternalLabel endpoint) {
    var result = new Block();
    result.Add(
        _F.CreateLabel(result,
                       CompilerContext.Current.GetUniqueName("L" + ordinal),
                       _tryDepth)
            .LabelStatement);
    result.Add(body);
  ...
  private Block CreateSwitch(List<Statement> handlers, InternalLabel endLabel,
                             InternalLocal switchVar) {
    var blocks = handlers
                     .Zip(Enumerable.Range(1, handlers.Count),
                          (s, i1) => SwitchBlock(s, i1, endLabel))
                     .ToArray();
    var resultSwitch = _F.CreateSwitch(
        _F.CreateLocalReference(switchVar),
  ...
  public override void OnGotoStatement(GotoStatement node) {
    ...
    var proxyLabel = _currentAwaitFinallyFrame.ProxyLabelIfNeeded(
        (InternalLabel)node.Label.Entity);
  ...
  public override void OnReturnStatement(ReturnStatement node) {
    ...
    var returnLabel = _currentAwaitFinallyFrame.ProxyReturnIfNeeded(
        _containingMethod, node.Expression, out returnValue);
    ...
    Statement result;
    if (returnExpr != null) {
      result = new Block(new ExpressionStatement(_F.CreateAssignment(
                             _F.CreateLocalReference(returnValue), returnExpr)),
                         _F.CreateGoto(returnLabel, _tryDepth));
    } else {
  ...
  private Statement UnpendException(InternalLocal pendingExceptionLocal) {
    // create a temp.
    // pendingExceptionLocal will certainly be captured, no need to access it
    // over and over.
    InternalLocal obj = _F.DeclareTempLocal(_containingMethod, _tss.ObjectType);
    var objInit =
        _F.CreateAssignment(_F.CreateLocalReference(obj),
                            _F.CreateLocalReference(pendingExceptionLocal));

    // throw pendingExceptionLocal;
    Statement rethrow = Rethrow(obj);

    return new Block(new ExpressionStatement(objInit),
                     new IfStatement(pendingExceptionLocal.Local.LexicalInfo,
                                     _F.CreateBoundBinaryExpression(
                                         _tss.BoolType,
                                         BinaryOperatorType.ReferenceInequality,
                                         _F.CreateLocalReference(obj),
                                         new NullLiteralExpression()),
                                     new Block(rethrow), null));
  }
  ...
  private Statement Rethrow(InternalLocal obj) {
    ...
    if (_exceptionDispatchInfoCapture != null &&
        _exceptionDispatchInfoThrow != null) {
      ...
      rethrow = new Block(
          ...
          new IfStatement(
              _F.CreateBoundBinaryExpression(
                  _tss.BoolType, BinaryOperatorType.ReferenceEquality,
                  _F.CreateLocalReference(ex), new NullLiteralExpression()),
              new Block(rethrow), null),
          // ExceptionDispatchInfo.Capture(pendingExceptionLocal).Throw()
          new ExpressionStatement(_F.CreateMethodInvocation(
              _F.CreateMethodInvocation(_exceptionDispatchInfoCapture,
                                        _F.CreateLocalReference(ex)),
  ...
  private Statement RewriteFinalizedRegion(TryStatement node) {
    ...
    if (currentAwaitCatchFrame != null) {
      var handledLabel = _F.CreateLabel(
          tryWithCatches, CompilerContext.Current.GetUniqueName("handled"),
          _tryDepth);
      var handlersList = currentAwaitCatchFrame.handlers;
      _tryDepth = node.GetAncestors<TryStatement>().Count() + 1;
      var handlers =
          new List<Statement> { _F.CreateGoto(handledLabel, _tryDepth) };
      for (int i = 0, l = handlersList.Count; i < l; i++) {
        handlers.Add(
            new Block(handlersList[i], _F.CreateGoto(handledLabel, _tryDepth)));
      }

      _containingMethod.Locals.Add(
          currentAwaitCatchFrame.pendingCaughtException.Local);
      _containingMethod.Locals.Add(currentAwaitCatchFrame.pendingCatch.Local);
      _containingMethod.Locals.AddRange(
          currentAwaitCatchFrame.GetHoistedLocals().Select(l => l.Local));
      ...
      tryWithCatches = new Block(
          ...
          tryWithCatches,
          CreateSwitch(handlers, handledLabel,
  ...
  public override void OnExceptionHandler(ExceptionHandler node) {
    ...
    var currentAwaitCatchFrame = _currentAwaitCatchFrame ??
                                 (_currentAwaitCatchFrame = new AwaitCatchFrame(
                                      _tss, _F, _containingMethod));

    var catchType = node.Declaration != null
                        ? (IType)node.Declaration.Type.Entity
                        : _tss.ObjectType;
    ...
    if (filterOpt == null) {
      ...
      catchAndPend = new ExceptionHandler {
        Declaration = new Declaration(
            catchTemp.Name,
            _F.CreateTypeReference(catchType)) { Entity = catchTemp },
        Block = new Block(new ExpressionStatement(storePending),
      ...
      var sourceOpt =
          decl != null && !string.IsNullOrEmpty(decl.Name) ? decl : null;
      ...
      var newFilter =
          sourceOpt == null
              ...
              : _F.CreateEvalInvocation(
                    storePending.LexicalInfo, storePending,
                    AssignCatchSource((Declaration)Visit(sourceOpt),
    ...
    if (filterOpt == null) {
      ...
      if (sourceOpt != null && sourceOpt.Entity != null) {
        Expression assignSource = AssignCatchSource(
            (Declaration)Visit(sourceOpt), currentAwaitCatchFrame);
  ...
  private IEnumerable<InternalLocal> LocalsUsedIn(ExceptionHandler handler) {
    var rc = new LocalReferenceCollector(
        _containingMethod.Locals.Select(l => (InternalLocal)l.Entity));
    handler.Accept(rc);
    return rc.Result;
  }

  private Expression AssignCatchSource(Declaration rewrittenSource,
    ...
    if (rewrittenSource != null) {
      ...
      assignSource = _F.CreateAssignment(
          ...
          _F.CreateCast((IType)rewrittenSource.Type.Entity,
                        _F.CreateLocalReference(
  ...
  public override void OnDeclaration(Declaration node) {
    ...
    if (catchFrame == null ||
        !catchFrame.TryGetHoistedLocal((InternalLocal)node.Entity,
                                       out hoistedLocal)) {
      base.OnDeclaration(node);
    }

    ReplaceCurrentNode(new Declaration(
        node.Name,
        _F.CreateTypeReference(hoistedLocal.Type)) { Entity = hoistedLocal });
  }
  ...
  private AwaitFinallyFrame PushFrame(TryStatement statement) {
    var newFrame = new AwaitFinallyFrame(_currentAwaitFinallyFrame,
                                         _analysis.Labels(statement), statement,
                                         _F, _tryDepth);
  ...
  private sealed class AwaitInFinallyAnalysis : LabelCollector {
    // all try blocks with yields in them and complete set of labels inside
    // those try blocks NOTE: non-yielding try blocks are transparently ignored
    // - i.e. their labels are included
    //       in the label set of the nearest yielding-try parent
    private Dictionary<TryStatement, HashSet<InternalLabel>>
    ...
    public bool FinallyContainsAwaits(TryStatement statement) {
      return _labelsInInterestingTry != null &&
             _labelsInInterestingTry.ContainsKey(statement);
    }
    ...
    internal bool CatchContainsAwait(ExceptionHandler node) {
      return _awaitContainingCatches != null &&
             _awaitContainingCatches.Contains(node);
    }
      ...
      if (_seenAwait) {
        ...
        if (labelsInInterestingTry == null) {
          _labelsInInterestingTry = labelsInInterestingTry =
              new Dictionary<TryStatement, HashSet<InternalLabel>>();
        }
  ...
  private sealed class AwaitFinallyFrame {
    ...
    // labels within this frame (branching to these labels does not go through
    // finally).
    ...
    public AwaitFinallyFrame(AwaitFinallyFrame parent,
                             HashSet<InternalLabel> labelsOpt,
                             TryStatement TryStatement, BooCodeBuilder builder,
                             int depth) {
    ...
    private bool IsRoot() { return ParentOpt == null; }
    ...
    public InternalLabel ProxyLabelIfNeeded(InternalLabel label) {
      // no need to proxy a label in the current frame or when we are at the
      // root
      ...
      if (proxyLabels == null) {
        this.proxyLabels = proxyLabels =
            new Dictionary<InternalLabel, InternalLabel>();
    ...
    public InternalLabel ProxyReturnIfNeeded(Method containingMethod,
                                             Node valueOpt,
      ...
      if (returnProxy == null) {
        returnProxyLabel = returnProxy = _builder.CreateLabel(
            valueOpt ?? containingMethod, "returnProxy", _tryDepth);
      }
      ...
      if (valueOpt != null) {
        ...
        if (retVal == null) {
          ...
          returnValue = retVal = _builder.DeclareTempLocal(
              containingMethod, ((ITypedEntity)valueOpt.Entity).Type);
        }
      }
  ...
  private sealed class AwaitCatchFrame {
    ...
    public AwaitCatchFrame(TypeSystemServices tss, BooCodeBuilder builder,
                           Method currentMethod) {
      pendingCaughtException =
          builder.DeclareTempLocal(currentMethod, tss.ObjectType);
    ...
    public void HoistLocal(InternalLocal local, BooCodeBuilder F) {
      if (!_hoistedLocals.Keys.Any(l => l.Name == local.Name &&
                                        l.Type == local.Type)) {
    ...
    public bool TryGetHoistedLocal(InternalLocal originalLocal,
                                   out InternalLocal hoistedLocal) {
