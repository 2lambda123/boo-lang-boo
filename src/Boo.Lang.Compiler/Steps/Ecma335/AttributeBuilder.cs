...
internal class AttributeBuilder {
  ...
  public void Build() {
    ...
    encoder.CustomAttributeSignature(out var fixedBuilder,
                                     out var namedBuilder);
    for (int i = 0; i < fixedParamCount; ++i) {
      EncodeValue(fixedBuilder.AddArgument(), _attr.Arguments[i],
                  constructorParams[i].Type);
    }
    if (varargs) {
      var arrBuilder = fixedBuilder.AddArgument().Vector().Count(
          _attr.Arguments.Count - fixedParamCount);
      var varargElementType =
          entity.GetParameters()[fixedParamCount].Type.ElementType;
      for (int i = fixedParamCount; i < _attr.Arguments.Count; ++i) {
        EncodeValue(arrBuilder.AddLiteral(), _attr.Arguments[i],
                    varargElementType);
      }
    }
    var enc = namedBuilder.Count(_attr.NamedArguments.Count);
    foreach (var nArg in _attr.NamedArguments) {
      enc.AddArgument(nArg.First.Entity is IField, out var typeEncoder,
                      out var nameEncoder, out var valueEncoder);
  ...
  private static readonly
      Dictionary<Type, Func<IBuilder, Ast.Attribute, TypeSystemBridge, bool>>
          _specialTypes = new() {
  ...
  public bool IsSpecial {
    get {
      var type = (((IConstructor)_attr.Entity).DeclaringType as ExternalType)
                     ?.ActualType;
  ...
  public bool HandleSpecialAttribute(IBuilder parent) {
    var ext =
        (((IConstructor)_attr.Entity).DeclaringType as ExternalType).ActualType;
  ...
  private static bool SpecialTypeLayout(IBuilder parent, Ast.Attribute attr,
                                        TypeSystemBridge ts) {
    var na = attr.NamedArguments;
    if (na != null) {
      var pack = (ushort)((na.FirstOrDefault(p => p.First.Matches(PACK_EXPR))
                               ?.Second as IntegerLiteralExpression)
                              ?.Value ??
                          0);
      var size = (ushort)((na.FirstOrDefault(p => p.First.Matches(SIZE_EXPR))
                               ?.Second as IntegerLiteralExpression)
                              ?.Value ??
                          0);

      if (pack != 0 || size != 0) {
        ts.AssemblyBuilder.AddTypeLayout((TypeDefinitionHandle)parent.Handle,
                                         pack, size);
      }
    }
    var arg = attr.Arguments[0];
    LayoutKind lk =
        arg.NodeType == NodeType.IntegerLiteralExpression
            ? (LayoutKind)Enum.ToObject(
                  typeof(LayoutKind),
                  (byte)((IntegerLiteralExpression)arg).Value)
  ...
  static void EncodeType(CustomAttributeElementTypeEncoder encoder,
                         IType type) {
    if ((type as ExternalType)?.ActualType == typeof(Type)) {
      encoder.SystemType();
    } else {
  ...
  private static PrimitiveSerializationTypeCode
  GetPrimitiveTypeCode(IType type) {
  ...
  private void EncodeValue(LiteralEncoder encoder, Expression arg,
    ...
    if (expectedType == _ts.ObjectTypeEntity) {
      ...
      EncodeScalarValue(encoder.Scalar(), arg, expectedType);
  }

  void EncodeScalarValue(ScalarEncoder encoder, Expression arg,
                         IType expectedType = null) {
    if (arg is TypeofExpression type) {
      encoder.SystemType(type.Type.Entity.FullName);
    } else if (arg is ReferenceExpression &&
               arg.ExpressionType == _ts.TypeTypeEntity) {
