...
internal class MethodBuilder : IBuilder {
  ...
  private readonly InstructionEncoder _il =
      new(new BlobBuilder(), new ControlFlowBuilder());
  ...
  private readonly MaxStackAnalyzer _maxStackAnalyzer =
      new(My<TypeSystemServices>.Instance);
  readonly Dictionary<LabelHandle, int> _labelPositions = new();
  readonly List<(int position, LabelHandle[] tokens, Blob buffer)> _switches =
      new();
  ...
  public MethodBuilder(InternalMethod method, MethodBodyStreamEncoder encoder,
                       MethodAttributes attrs, bool debug,
                       TypeSystemBridge typeSystem)
      : this(method, encoder, attrs, debug, typeSystem,
             typeSystem.ReserveMethod(method)) {}

  public MethodBuilder(InternalMethod method, MethodBodyStreamEncoder encoder,
                       MethodAttributes attrs, bool debug,
                       TypeSystemBridge typeSystem, EntityHandle handle) {
  ...
  public void Switch(LabelHandle[] labels) {
    ...
    var tokenBlob =
        _il.CodeBuilder.ReserveBytes((labels.Length + 1) * sizeof(int));
  ...
  public void CallArrayMethod(IType arrayType, string methodName,
                              IType returnType, IType[] parameterTypes) {
    var handle = _typeSystem.GetArrayMethod(arrayType, methodName, returnType,
                                            parameterTypes);
    _il.OpCode(ILOpCode.Call);
    _il.Token(handle);
    _maxStackAnalyzer.CallArrayMethod(parameterTypes.Length,
                                      returnType != _typeSystem.VoidTypeEntity);
  }

  public void LoadConstant(long value) {
    _il.LoadConstantI8(value);
    _maxStackAnalyzer.Operation(ILOpCode.Ldc_i8);
    _maxStackAnalyzer.Operation(ILOpCode.Ldc_i8);
  }
  ...
  public int AddLocal(IType type, string name = null) {
    ...
    if (_isDebug) {
      var lvar = _typeSystem.DebugBuilder.AddLocalVariable(
          name?.Contains('$') == true ? LocalVariableAttributes.DebuggerHidden
                                      : LocalVariableAttributes.None,
          _locals.Count,
          name != null ? _typeSystem.DebugBuilder.GetOrAddString(name)
                       : default);
      if (_firstVar.IsNil) {
        _firstVar = lvar;
      }
  ...
  private readonly Stack<(LabelHandle start, LabelHandle end)> _tryLabels =
      new();
  private readonly Stack<(LabelHandle start, LabelHandle end, HandlerType type)>
      _handlers = new();
  private readonly Stack<LabelHandle> _exceptionExitPoints = new();
  private readonly Stack<EntityHandle> _catchType = new();

  private enum HandlerType { Except, Fail, Finally, FilterExpr, FilterBody }
...
#pragma warning disable IDE0042 // Deconstruct variable declaration
  public void EndFail() {
    ...
    if (frame.type != HandlerType.Fail) {
      throw new InvalidOperationException(
          "EndFail called without a matching call to BeginFail");
    }
    OpCode(ILOpCode.Endfinally);
    MarkLabel(frame.end);
    var tryFrame = _tryLabels.Peek();
    _il.ControlFlowBuilder.AddFaultRegion(tryFrame.start, tryFrame.end,
  ...
  public void EndFinally() {
    ...
    if (frame.type != HandlerType.Finally) {
      throw new InvalidOperationException(
          "EndFinally called without a matching call to BeginFinally");
    }
    OpCode(ILOpCode.Endfinally);
    MarkLabel(frame.end);
    var tryFrame = _tryLabels.Peek();
    _il.ControlFlowBuilder.AddFinallyRegion(tryFrame.start, tryFrame.end,
                                            frame.start, frame.end);
  }
  ...
  public void BeginFilterBody() {
    ...
    if (frame.type != HandlerType.FilterExpr) {
      throw new InvalidOperationException(
          "BeginFilterBody called without a matching call to BeginFilter");
    }
  ...
  public void EndFilter() {
    ...
    if (bodyFrame.type != HandlerType.FilterBody) {
      throw new InvalidOperationException(
          "EndFilter called without a matching call to BeginFilterBody");
    }
    var exprFrame = _handlers.Pop();
    if (exprFrame.type != HandlerType.FilterExpr) {
      throw new InvalidOperationException(
          "EndFilter called without a matching call to BeginFilter");
    }
    Branch(ILOpCode.Leave, _exceptionExitPoints.Peek());
    MarkLabel(bodyFrame.end);
    var tryFrame = _tryLabels.Peek();
    _il.ControlFlowBuilder.AddFilterRegion(tryFrame.start, tryFrame.end,
                                           bodyFrame.start, bodyFrame.end,
                                           exprFrame.start);
  }
  ...
  public void EndCatchBlock() {
    ...
    if (frame.type != HandlerType.Except) {
      throw new InvalidOperationException(
          "EndCatchBlock called without a matching call to BeginCatchBlock");
    }
    Branch(ILOpCode.Leave, _exceptionExitPoints.Peek());
    MarkLabel(frame.end);
    var type = _catchType.Pop();
    var tryFrame = _tryLabels.Peek();
    _il.ControlFlowBuilder.AddCatchRegion(tryFrame.start, tryFrame.end,
...
#pragma warning restore IDE0042 // Deconstruct variable declaration
  ...
  MethodVisibilityAttributesFor(TypeMember method) {
    if (method.IsPublic)
      return MethodAttributes.Public;
    if (method.IsProtected)
      return method.IsInternal ? MethodAttributes.FamORAssem
  ...
  private (BlobHandle sig, ParameterHandle[] pHandles) BuildSignature() {
    ...
    var sig = builder.MethodSignature(
        ...
        _method.GenericInfo?.GenericParameters?.Length ?? 0, !_method.IsStatic);
    sig.Parameters(_params.Count, out var ret, out var parms);
    var returnType = _method.ReturnType;
    if ((_method.IsPInvoke && TypeSystemServices.IsUnknown(returnType)) ||
        returnType == _typeSystem.VoidTypeEntity) {
      ret.Void();
    } else {
    ...
    foreach (var param in _params) {
      ...
      _typeSystem.EncodeType(parms.AddParameter().Type(entity.IsByRef),
                             entity.Type);
    }
  ...
  private (int offset, StandaloneSignatureHandle handle) BuildBody() {
    ...
    if (_locals.Count > 0) {
      ...
      sigHandle = _asmBuilder.AddStandaloneSignature(
          _asmBuilder.GetOrAddBlob(localBuilder.Builder));
    }
  ...
  public ParameterBuilder DefineParameter(IParameter param, int index,
  ...
  public GenericTypeParameterBuilder[] DefineGenericParameters(
      IEnumerable<IGenericParameter> parameters) {
    if (_genParams != null) {
      throw new EcmaBuildException(
          "Generic parameters have already been defined for this method.");
    }
    var result =
        parameters
            .Select(p => new GenericTypeParameterBuilder(p, this, _typeSystem))
            .ToArray();
    _genParams = result;
    return result;
  }
  ...
  private void BuildSequencePoints(StandaloneSignatureHandle signature) {
    ...
    var multiDoc = arr.Select(kvp => kvp.Value.FileName)
    ...
    var first =
        arr.FirstOrDefault(sp => !string.IsNullOrEmpty(sp.Value.FileName))
            .Value;
    if (first == null) {
      return;
    }
    ...
    for (int i = 1; i < arr.Length; i++) {
      ...
      builder.WriteCompressedInteger(
          Math.Max(next.Value.Line - last.Value.Line, 1));
      builder.WriteCompressedSignedInteger(next.Value.Column -
                                           last.Value.Column);
      last = next;
    }
  ...
  public void Build() {
    ...
    var name = _method is IConstructor
                   ...
                   : (explicitInfo != null
                          ? explicitInfo.InterfaceType.Name + "." + _method.Name
                          : _method.Name);
    var isRuntime =
        ((Method)_method.Node)
            .ImplementationFlags.HasFlag(MethodImplementationFlags.Runtime);
    var handle = _asmBuilder.AddMethodDefinition(
        ...
        _asmBuilder.GetOrAddString(name), sig,
        isRuntime || _method.IsAbstract ? -1 : bodyOffset,
        pHandles.Length > 0
            ? pHandles[0]
            : MetadataTokens.ParameterHandle(
                  _asmBuilder.GetRowCount(TableIndex.Param) + 1));
    if (handle != _handle) {
      throw new EcmaBuildException(
          $"Method build handle {handle} does not match reserved handle {_handle}.");
    }
    if (_isDebug) {
      BuildSequencePoints(localSig);
      var db = _typeSystem.DebugBuilder;
      db.AddLocalScope(handle, _typeSystem.NullImportHandle, _firstVar, default,
                       bodyOffset, _il.CodeBuilder.Count);
    }
  }
}
}
