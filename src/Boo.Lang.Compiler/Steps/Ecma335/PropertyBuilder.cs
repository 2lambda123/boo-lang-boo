...
internal class PropertyBuilder : IBuilder {
  ...
  public PropertyBuilder(IProperty entity, PropertyAttributes attrs,
  ...
  internal void SetGetMethod(MethodBuilder builder) { _getter = builder; }

  internal void SetSetMethod(MethodBuilder builder) { _setter = builder; }
  ...
  public void Build() {
    ...
    var handle = asm.AddProperty(_attrs, asm.GetOrAddString(_entity.Name),
                                 asm.GetOrAddBlob(sig.Builder));
    if (handle != Handle) {
    ...
    if (_getter != null) {
      asm.AddMethodSemantics(handle, MethodSemanticsAttributes.Getter,
                             (MethodDefinitionHandle)_getter.Handle);
    }
    if (_setter != null) {
      asm.AddMethodSemantics(handle, MethodSemanticsAttributes.Setter,
                             (MethodDefinitionHandle)_setter.Handle);
    }
  }
}
}
