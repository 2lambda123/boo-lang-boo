...
internal class ParameterBuilder : IBuilder {
  ...
  public ParameterBuilder(IParameter param, int index,
  ...
  public void Build() {
    ...
    var handle =
        asm.AddParameter(_attributes, asm.GetOrAddString(_param.Name), _index);
    if (handle != Handle) {
      throw new EcmaBuildException(
          $"Parameter build handle {handle} does not match reserved handle {Handle}.");
    }
  }
}
}
