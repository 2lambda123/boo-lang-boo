...
internal class TypeSystemBridge {
  ...
  private readonly Dictionary<EntityHandle, IMethod> _reversemethodLookup =
      new();
  ...
  private readonly
      List<(EntityHandle parent, GenericTypeParameterBuilder builder)>
          _genParamBuilders = new();
  private readonly List<(EntityHandle parent, TypeBuilder builder)>
      _typeBuilders = new();
  ...
  public TypeSystemBridge(MetadataBuilder builder, TypeSystemServices tss,
                          Func<IType, Expression, object> getValue) {
    _tss = tss;
    ...
    var runtimeName = loadedAssemblies.Single(a => a.GetName().Name ==
    "System.Runtime").GetName(); var privateAsm = loadedAssemblies.Single(a =>
    a.GetName().Name == "System.Private.CoreLib"); var handle =
    HandleForAssemblyName(runtimeName); _assemblyLookup[privateAsm] = handle;
    */
  ...
  public EntityHandle LookupType(IType type) {
    ...
    if (type.IsArray || type is AbstractGenericParameter ||
    ...
    throw new EcmaBuildException(
        $"Unable to lookup handle for type {type.QualifiedName()} of entity type {type.GetType()}.");
  }

  private readonly HashSet<IType> _eventMappedTypes = new();

  internal void EnsureEventMapForType(IType type,
                                      EventDefinitionHandle handle) {
    if (!_eventMappedTypes.Contains(type)) {
      _eventMappedTypes.Add(type);
      AssemblyBuilder.AddEventMap((TypeDefinitionHandle)LookupType(type),
                                  handle);
    }
  }
  ...
  internal void EnsurePropertyMapForType(IType type,
                                         PropertyDefinitionHandle handle) {
    if (!_propertyMappedTypes.Contains(type)) {
      _propertyMappedTypes.Add(type);
      AssemblyBuilder.AddPropertyMap((TypeDefinitionHandle)LookupType(type),
                                     handle);
    }
  }
  ...
  private EntityHandle HandleForAssemblyName(AssemblyName asmName) {
    ...
    return AssemblyBuilder.AddAssemblyReference(
        AssemblyBuilder.GetOrAddString(asmName.Name), asmName.Version,
        AssemblyBuilder.GetOrAddString(asmName.CultureName),
        AssemblyBuilder.GetOrAddBlob(keyOrToken),
        ConvertAssemblyNameFlags(asmName.Flags), default);
  }

  public static AssemblyFlags
  ConvertAssemblyNameFlags(AssemblyNameFlags flags) {
  ...
  private EntityHandle LookupTypeSpec(IType type) {
    var encoder =
        new BlobEncoder(new BlobBuilder()).TypeSpecificationSignature();
    EncodeType(encoder, type);

    var result = AssemblyBuilder.AddTypeSpecification(
        AssemblyBuilder.GetOrAddBlob(encoder.Builder));
    _typeLookup.Add(type, result);
    return result;
  }

  internal void EncodeTypeModified(SignatureTypeEncoder enc, IType type,
                                   IType modifier, bool required) {
  ...
  private void EncodeUnconstructedType(SignatureTypeEncoder enc, IType type) {
    ...
    var argEncoder = enc.GenericInstantiation(
        LookupType(type), gi.GenericParameters.Length, type.IsValueType);
    foreach (var param in gi.GenericParameters) {
      argEncoder.AddArgument().GenericTypeParameter(
          param.GenericParameterPosition);
    }
  }

  private void EncodeConstructedType(SignatureTypeEncoder enc,
                                     IConstructedTypeInfo ci) {
    var argEncoder = enc.GenericInstantiation(LookupType(ci.GenericDefinition),
  ...
  private static readonly Dictionary<string, PrimitiveTypeCode>
      _primitiveTypes = new() {
  ...
  public EntityHandle LookupMethod(IMethodBase method) {
    ...
    throw new EcmaBuildException(
        $"Unable to lookup handle for method {method.FullName} of entity type {method.GetType()}.");
  }

  private EntityHandle LookupExternalMethod(IMethod method) {
    var typeRef = LookupType(method.DeclaringType);
    var name = method.EntityType == EntityType.Constructor
                   ? (method.IsStatic ? ".cctor" : ".ctor")
                   : method.Name;
    var result = AssemblyBuilder.AddMemberReference(
        typeRef, AssemblyBuilder.GetOrAddString(name),
  ...
  private BlobHandle GetMethodSignature(IMethod method) {
    ...
    var sig = enc.MethodSignature(
        genericParameterCount: method.ConstructedInfo?.GenericArguments
                ?.Length ??
            method.GenericInfo?.GenericParameters?.Length ?? 0,
  ...
  private EntityHandle LookupMethodSpec(IMethod m) {
    if (!_methodLookup.TryGetValue(m, out var result)) {
      ...
      var enc = new BlobEncoder(new BlobBuilder())
                    .MethodSpecificationSignature(args.Length);
      foreach (var arg in args) {
        EncodeType(enc.AddArgument(), arg);
      }
      result = AssemblyBuilder.AddMethodSpecification(
          defHandle, AssemblyBuilder.GetOrAddBlob(enc.Builder));
  ...
  private readonly Dictionary<(IType type, string name), EntityHandle>
      _arrayMethods = new();

  public EntityHandle GetArrayMethod(IType arrayType, string methodName,
    ...
    if (!_arrayMethods.TryGetValue(key, out var result)) {
      ...
      var sig =
          builder.MethodSignature(SignatureCallingConvention.Default, 0, true);
      ...
      result = AssemblyBuilder.AddMemberReference(
          typ, AssemblyBuilder.GetOrAddString(methodName),
  ...
  public IMethodBase MethodOf(IType typ, string name, params IType[] args) {
    var method = typ.GetMembers().OfType<IMethodBase>().Single(
        m => m.Name == name && ParamsMatch(m.GetParameters(), args));
    return method;
  }
  ...
  public IMethodBase MethodOf<T>(string name,
                                 params Type[] args) => MethodOf(typeof(T),
                                                                 name, args);

  public IMethodBase MethodOf<T>(string name, params IType[] args) =>
      MethodOf(_tss.Map(typeof(T)), name, args);

  public IConstructor ConstructorOf<T>(params Type[] args) => (IConstructor)
      MethodOf<T>("constructor", args);

  public IConstructor ConstructorOf<T>(params IType[] args) => (IConstructor)
      MethodOf<T>("constructor", args);
  ...
  internal EntityHandle FieldOf(IType type, string fieldName) {
    var field =
        type.GetMembers().OfType<IField>().Single(f => f.Name == fieldName);
  ...
  internal EntityHandle LookupField(IField field) {
    if (!_fieldLookup.TryGetValue(field, out var result)) {
      ...
      var sigField = type.ConstructedInfo == null
                         ? field
                         : type.ConstructedInfo.GenericDefinition.GetMembers()
                               .OfType<IField>()
      ...
      result = AssemblyBuilder.AddMemberReference(
          typeHandle, AssemblyBuilder.GetOrAddString(field.Name),
          AssemblyBuilder.GetOrAddBlob(sig.Builder));
  ...
  internal object GetExpressionValue(Expression expr, IType expectedType) =>
      _getValue(expectedType ?? expr.ExpressionType, expr);

  internal CustomAttributeHandle SetCustomAttribute(IBuilder builder,
                                                    AttributeBuilder attr) {
    if (attr.IsSpecial && attr.HandleSpecialAttribute(builder)) {
      return default;
    } else
      return SetCustomAttribute(builder.Handle, attr);
  }

  internal CustomAttributeHandle SetCustomAttribute(EntityHandle handle,
                                                    AttributeBuilder attr) {
    if (attr.Handle.IsNil) {
      attr.Build();
    }
    return AssemblyBuilder.AddCustomAttribute(handle, attr.ConstructorHandle,
                                              attr.Handle);
  }

  internal MethodSpecificationHandle
  ConstructMethodOfGenericType(IMethod baseMethod, IType type) {
    var constructedMethod =
        (IMethod)baseMethod.DeclaringType.GenericInfo.ConstructType(type)
            .ConstructedInfo.Map(baseMethod);
    return (MethodSpecificationHandle)LookupMethod(constructedMethod);
  }

  internal void RegisterGenericParameter(EntityHandle parent,
                                         GenericTypeParameterBuilder builder) {
    _genParamBuilders.Add((parent, builder));
  }

  internal void BuildGenericParameters() {
    foreach (var (handle, builder) in _genParamBuilders.OrderBy(
                 p => CodedIndex.TypeOrMethodDef(p.parent))) {
  ...
  internal void BuildNestedTypes() {
    foreach (var (handle, builder) in _typeBuilders.OrderBy(
                 p => CodedIndex.TypeDefOrRef(p.builder.Handle))) {
      var idx = CodedIndex.TypeDefOrRef(handle);
      AssemblyBuilder.AddNestedType((TypeDefinitionHandle)builder.Handle,
  ...
  internal DocumentHandle LookupDocument(string name) {
    ...
    if (!_documentLookup.TryGetValue(name, out var result)) {
      ...
      result = DebugBuilder.AddDocument(handle, default, default,
                                        _booLangGuidHandle);
