...
internal class TypeBuilder : IBuilder {
  ...
  private readonly List<KeyValuePair<EntityHandle, EntityHandle>>
      _explicitImpls = new();
  private readonly TypeSystemBridge _typeSystem;

  private GenericTypeParameterBuilder[] _genParams;

  public TypeBuilder(TypeDefinition type, BlobBuilder ilBuilder,
                     TypeSystemBridge typeSystem, bool isDebug)
      : this(type, ilBuilder, GetTypeAttributes(type), typeSystem, isDebug) {}

  private TypeBuilder(TypeDefinition type, BlobBuilder ilBuilder,
                      TypeAttributes attrs, TypeSystemBridge typeSystem,
                      bool isDebug) {
  ...
  public TypeBuilder DefineNestedType(TypeDefinition type) {
    var result = new TypeBuilder(
        type, _ilBuilder, GetNestedTypeAttributes(type), _typeSystem, _isDebug);
  ...
  public FieldBuilder DefineField(IField field, IType type,
  ...
  public MethodBuilder DefineMethod(InternalMethod method,
                                    MethodAttributes attrs) {
    var result =
        new MethodBuilder(method, new MethodBodyStreamEncoder(_ilBuilder),
                          attrs, _isDebug, _typeSystem);
    _methods.Add(result);
    return result;
  }
  ...
  public PropertyBuilder DefineProperty(IProperty prop,
  ...
  private static string AnnotateGenericTypeName(TypeDefinition typeDef,
                                                string name) =>
      typeDef.HasGenericParameters ? $"{name}`{typeDef.GenericParameters.Count}"
                                   : name;
  ...
  static TypeAttributes GetExtendedTypeAttributes(TypeAttributes attributes,
                                                  TypeMember type) {
  ...
  internal void SetLayout(TypeAttributes layout) {
    if ((layout & TypeAttributes.LayoutMask) != layout) {
      throw new EcmaBuildException(
          $"Non-layout value '{layout}' passed to SetLayout");
    }
  ...
  private void EmitBaseTypes() {
    foreach (var baseType in _type.BaseTypes) {
      ...
      if ((type.ConstructedInfo != null &&
           type.ConstructedInfo.GenericDefinition.IsClass) ||
          (type.IsClass)) {
        _parent = type;
      } else {
  ...
  internal void AddMethodImplementation(EntityHandle ifaceInfo,
                                        EntityHandle implInfo) {
  ...
  public void Build() {
    ...
    var handle = asm.AddTypeDefinition(
        _attrs,
        _type.EnclosingNamespace == null
            ? default
            : asm.GetOrAddString(_type.EnclosingNamespace.Name),
        asm.GetOrAddString(_name),
        _parent == null ? (_type.NodeType == NodeType.InterfaceDefinition
                               ? default
                               : _typeSystem.ObjectType)
                        : _typeSystem.LookupType(_parent),
        firstField != null ? (FieldDefinitionHandle)firstField.Handle
                           : MetadataTokens.FieldDefinitionHandle(
                                 asm.GetRowCount(TableIndex.Field) + 1),
        firstMethod != null ? (MethodDefinitionHandle)firstMethod.Handle
                            : MetadataTokens.MethodDefinitionHandle(
                                  asm.GetRowCount(TableIndex.MethodDef) + 1));
    if (handle != _handle) {
      throw new EcmaBuildException(
          $"Type handle {handle} does not match reserved handle {_handle}.");
    }
    ...
    foreach (var iHandle in _interfaceImplementations
                 .Select(_typeSystem.LookupType)
                 .OrderBy(CodedIndex.TypeDefOrRefOrSpec)) {
  ...
  internal GenericTypeParameterBuilder[] DefineGenericParameters(
      IEnumerable<IGenericParameter> parameters) {
    if (_genParams != null) {
      throw new EcmaBuildException(
          "Generic parameters have already been defined for this type.");
    }
    var result =
        parameters
