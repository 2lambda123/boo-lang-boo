...
public class NestMultipleExceptionHandlers : AbstractFastVisitorCompilerStep {
  ...
  private static void NestTryStatement(TryStatement node) {
    var firstEHLine =
        node.ExceptionHandlers?.First?.LexicalInfo?.Line ?? int.MinValue;
  ...
  private static void NestTryStatementInEH(TryStatement node) {
    var newTry = new TryStatement(node.LexicalInfo) {
      ProtectedBlock = node.ProtectedBlock, EnsureBlock = node.EnsureBlock,
      FailureBlock = node.FailureBlock
    };
    if (node.ExceptionHandlers?.Count > 1 &&
        node.ExceptionHandlers.Any(eh => eh.FilterCondition != null)) {
  ...
  private static void DivideExceptionHandlers(TryStatement oldTry,
                                              TryStatement newTry) {
    ...
    if (oldTry.ExceptionHandlers.First.FilterCondition != null) {
      ...
      newTry.ExceptionHandlers.AddRange(
          oldTry.ExceptionHandlers.TakeWhile(e => e.FilterCondition == null));
      oldEH.AddRange(
          oldTry.ExceptionHandlers.SkipWhile(e => e.FilterCondition == null));
    }
  ...
  private static void NestTryStatementInEnsure(TryStatement node) {
    var newTry = new TryStatement(node.LexicalInfo) {
      ProtectedBlock = node.ProtectedBlock, FailureBlock = node.FailureBlock,
      ExceptionHandlers = node.ExceptionHandlers
    };
    node.ProtectedBlock = new Block(node.ProtectedBlock.LexicalInfo);
    node.ProtectedBlock.Statements.Add(newTry);
