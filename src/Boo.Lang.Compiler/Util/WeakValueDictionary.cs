...
/// <summary>
/// A dictionary in which the values are weak references. Written by DLP for
/// SWIG.
/// </summary>
/// <remarks>
...
/// not present.
///
/// This class "cleans up" periodically by removing entries with
/// garbage-collected values. Cleanups only occur occasionally, and only when
/// the dictionary is accessed; Accessing it (for read or write) more often
/// results in more frequent cleanups.
///
/// Watch out! The following interface members are not implemented:
/// IDictionary.Values, ICollection.Contains, ICollection.CopyTo,
/// ICollection.Remove. Also, the dictionary is NOT MULTITHREAD-SAFE.
/// </remarks>
...
#region IDictionary < TKey, TValue> Members
    ...
    if (_dict.TryGetValue(key, out WeakReference<TValue> wr)) {
      if (wr.TryGetTarget(out _))
        throw new ArgumentException("An element with the same key already " +
                                    "exists in this WeakValueDictionary");
      else
        wr.SetTarget(value);
    } else
  ...
  public TValue this[TKey key] {
    ...
    set { _dict[key] = new WeakReference<TValue>(value); }
  }
