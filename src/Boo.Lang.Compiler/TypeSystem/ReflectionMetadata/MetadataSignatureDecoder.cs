...
internal class MetadataGenericContext {
  public MetadataGenericContext(IEnumerable<IType> typeParameters,
                                IEnumerable<IType> methodParameters) {
    MethodParameters = methodParameters == null ? new List<IType>()
                                                : methodParameters.ToList();
    TypeParameters =
        typeParameters == null ? new List<IType>() : typeParameters.ToList();
  }
...
internal class MetadataSignatureDecoder
    : ISignatureTypeProvider<IType, MetadataGenericContext>,
      ICustomAttributeTypeProvider<IType> {
  private readonly MetadataTypeSystemProvider _tss;
  private readonly MetadataReader _reader;

  public MetadataSignatureDecoder(MetadataTypeSystemProvider provider,
  ...
  public IType GetGenericInstantiation(IType genericType,
                                       ImmutableArray<IType> typeArguments) {
    return genericType.GenericInfo.ConstructType(typeArguments.ToArray());
  }

  public IType GetGenericMethodParameter(MetadataGenericContext context,
                                         int index) {
    if (context == null)
      return null;
  ...
  public IType GetGenericTypeParameter(MetadataGenericContext context,
                                       int index) {
    if (context == null)
      return null;
  ...
  public IType GetModifiedType(IType modifier, IType unmodifiedType,
                               bool isRequired) {
    throw new NotImplementedException();
  }
  ...
  public IType GetPrimitiveType(PrimitiveTypeCode typeCode) {
    ...
    default:
      throw new ArgumentException(
          string.Format("Unknown primitive type code: {0}", typeCode));
    }
  }
  ...
  public IType GetTypeFromDefinition(MetadataReader reader,
  ...
  public IType GetTypeFromReference(MetadataReader reader,
  ...
  public IType GetTypeFromSpecification(MetadataReader reader,
  ...
  public IType GetSystemType() { return _tss.Map(typeof(System.Type)); }
  ...
  public PrimitiveTypeCode GetUnderlyingEnumType(IType type) {
    ...
    var fieldType =
        type.GetMembers().OfType<IField>().Single(f => !f.IsStatic).Type;
  ...
  public bool IsSystemType(IType type) { return type.Equals(GetSystemType()); }
}
}
