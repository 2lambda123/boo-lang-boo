...
internal enum WinErrorCode : ushort { FileNotFound = 2 }

internal sealed class GacAssemblyReferenceResolver
    : IAssemblyReferenceResolver {
  private IAssemblyCache globalAssemblyCache;
  private bool fusionNotAvailable;

  public bool TryGetAssemblyPath(AssemblyReferenceData assemblyReference,
                                 out string path) {
    if (fusionNotAvailable || assemblyReference.Version == null ||
        assemblyReference.PublicKeyToken == null) {
      path = null;
      return false;
    }
    ...
    try {
      if (TryLocate(assemblyReference.FullName, out path))
        return true;
    } catch (Exception ex) when (ex is DllNotFoundException          // CoreCLR
  ...
  private bool TryLocate(string name, out string path) {
    ...
    var result = globalAssemblyCache.QueryAssemblyInfo(
        QUERYASMINFO_FLAG.VALIDATE, name, ref assemblyInfo);
    if (result.IsError) {
      if (result.Code != WinErrorCode.FileNotFound)
        throw result.CreateException();
      path = null;
      return false;
    }

    path = assemblyInfo.pszCurrentAssemblyPathBuf.Substring(
        0, assemblyInfo.cchBuf - 1);
    return true;
  }

  [DllImport("fusion.dll", PreserveSig = false)]
  private static extern void CreateAssemblyCache(out IAssemblyCache ppAsmCache,
                                                 uint dwReserved);
  ...
  private interface IAssemblyCache {
    void
    UninstallAssembly(uint dwFlags,
                      [MarshalAs(UnmanagedType.LPWStr)] string pszAssemblyName,
                      IntPtr pRefData, out uint pulDisposition);
    [PreserveSig]
    HResult
    QueryAssemblyInfo(QUERYASMINFO_FLAG dwFlags,
                      [MarshalAs(UnmanagedType.LPWStr)] string pszAssemblyName,
                      ref ASSEMBLY_INFO pAsmInfo);
    void CreateAssemblyCacheItem(
        uint dwFlags, IntPtr pvReserved, out IntPtr ppAsmItem,
        [MarshalAs(UnmanagedType.LPWStr)] string pszAssemblyName);
...
#pragma warning disable IDE1006 // Naming Styles
  ...
  [Flags]
  private enum QUERYASMINFO_FLAG : uint { VALIDATE = 1, GETSIZE = 2 }
