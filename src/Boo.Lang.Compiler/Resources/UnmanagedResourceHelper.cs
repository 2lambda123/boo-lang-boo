...
internal class Win32Resource {
  ...
  internal Win32Resource(byte[] data, DWORD codePage, DWORD languageId, int id,
                         string name, int typeId, string typeName) {
    _data = data;
    _codePage = codePage;
...
internal static class VersionHelper {
  ...
  /// <param name="version">If parsing succeeds, the parsed version. Otherwise a
  /// version that represents as much of the input as could be parsed
  /// successfully.</param> <returns>True when parsing succeeds completely (i.e.
  /// every character in the string was consumed), false otherwise.</returns>
  internal static bool TryParse(string s, out Version version) {
    return TryParse(s, allowWildcard: false, maxValue: ushort.MaxValue,
                    allowPartialParse: true, version: out version);
  }

  /// <summary>
  /// Parses a version string of the form "major [ '.' minor [ '.' ( '*' | (
  /// build [ '.' ( '*' | revision ) ] ) ) ] ]" as accepted by
  /// System.Reflection.AssemblyVersionAttribute.
  /// </summary>
  /// <param name="s">The version string to parse.</param>
  /// <param name="allowWildcard">Indicates whether or not a wildcard is
  /// accepted as the terminal component.</param> <param name="version"> If
  /// parsing succeeded, the parsed version. Otherwise a version instance with
  /// all parts set to zero. If <paramref name="s"/> contains * the version
  /// build and/or revision numbers are set to <see cref="ushort.MaxValue"/>.
  /// </param>
  /// <returns>True when parsing succeeds completely (i.e. every character in
  /// the string was consumed), false otherwise.</returns>
  internal static bool TryParseAssemblyVersion(string s, bool allowWildcard,
                                               out Version version) {
    return TryParse(s, allowWildcard: allowWildcard,
                    maxValue: ushort.MaxValue - 1, allowPartialParse: false,
                    version: out version);
  }
  ...
  /// </summary>
  /// <param name="s">The version string to parse.</param>
  /// <param name="allowWildcard">Indicates whether or not we're parsing an
  /// assembly version string. If so, wildcards are accepted and each component
  /// must be less than 65535.</param> <param name="maxValue">The maximum value
  /// that a version component may have.</param> <param
  /// name="allowPartialParse">Allow the parsing of version elements where
  /// invalid characters exist. e.g. 1.2.2a.1</param> <param name="version"> If
  /// parsing succeeded, the parsed version. When <paramref
  /// name="allowPartialParse"/> is true a version with values up to the first
  /// invalid character set. Otherwise a version with all parts set to zero. If
  /// <paramref name="s"/> contains * and wildcard is allowed the version build
  /// and/or revision numbers are set to <see cref="ushort.MaxValue"/>.
  /// </param>
  /// <returns>True when parsing succeeds completely (i.e. every character in
  /// the string was consumed), false otherwise.</returns>
  private static bool TryParse(string s, bool allowWildcard, ushort maxValue,
                               bool allowPartialParse, out Version version) {
    ...
    // If the wildcard is being used, the first two elements must be specified
    // explicitly, and the last must be a exactly single asterisk without
    // whitespace.
    ...
    for (int i = 0; i < lastExplicitValue; i++) {
      ...
      if (!ushort.TryParse(elements[i], NumberStyles.None,
        ...
        // There could be an invalid character in the input so check for the
        // presence of one and parse up to that point. examples of invalid
        // characters are alphas and punctuation
        ...
        if (!invalidFormat) {
          // if we made it here then there weren't any alpha or punctuation
          // chars in the input so the element is either greater than
          // ushort.MaxValue or possibly a fullwidth unicode digit.
          if (TryGetValue(elements[i], out values[i])) {
            // For this scenario the old compiler would continue processing the
            // remaining version elements so continue processing
            continue;
          }
        }
  ...
  private static bool TryGetValue(string s, out ushort value) {
    if (System.Numerics.BigInteger.TryParse(s, NumberStyles.None,
                                            CultureInfo.InvariantCulture,
                                            out var number)) {
      // The old compiler would take the 16 least significant bits and use their
      // value as the output so we'll do that too.
      value = (ushort)(number % 65536);
      return true;
    }

    // One case that will cause us to end up here is when the input is a
    // Fullwidth unicode digit so we'll always return zero
    value = 0;
    return false;
  }

  /// <summary>
  /// If build and/or revision numbers are 65535 they are replaced with
  /// time-based values.
  /// </summary>
  public static Version
  GenerateVersionFromPatternAndCurrentTime(DateTime time, Version pattern) {
    ...
    // is the number of seconds since midnight local time (without taking into
    // account time zone adjustments for daylight saving time), divided by 2."
    ...
    if (pattern.Build == ushort.MaxValue) {
      ...
      return new Version(pattern.Major, pattern.Minor, (ushort)build,
                         (ushort)revision);
    } else {
      return new Version(pattern.Major, pattern.Minor, pattern.Build,
                         (ushort)revision);
    }
  }
}
...
internal static class UnamangedResourceHelper {
  ...
  internal static void AppendIconToResourceStream(Stream resStream,
                                                  Stream iconStream) {
    const string IconStreamUnexpectedFormat =
    ...
    // of the data here:
    // EDMAURER: PNG compressed icons must be treated differently. Do what has
    // always been done for uncompressed icons. Assume modern, compressed icons
    // set the ICONDIRENTRY fields correctly. if (*(DWORD*)icoBuffer ==
    ...
    for (ushort i = 0; i < count; i++) {
      ...
      */
      ...
      resStream.Position =
          (resStream.Position + 3) & ~3; // headers begin on 4-byte boundaries.
      ...
      resWriter.Write((WORD)0xFFFF);
      resWriter.Write(
          (WORD)(i + 1)); // EDMAURER this is not general. Implies you can only
                          // append one icon to the resources.
      // This icon ID would seem to be global among all of the icons not just
      // this group. Zero appears to not be an acceptable ID. Note that this ID
      ...
      iconStream.Position = iconDirEntries[i].dwImageOffset;
      resWriter.Write(
          iconReader.ReadBytes(checked((int)iconDirEntries[i].dwBytesInRes)));
    }
    ...
    */
    ...
    resWriter.Write(
        (DWORD)(3 * sizeof(WORD) + count * /*sizeof(ICONRESDIR)*/ 14));
    resWriter.Write((DWORD)0x00000020);
    resWriter.Write((WORD)0xFFFF);
  ...
  internal static void AppendVersionToResourceStream(
      Stream resStream, bool isDll,
      string fileVersion, // should be [major.minor.build.rev] but doesn't have
                          // to be
      string originalFileName, string internalName,
      string productVersion,   // 4 ints
      Version assemblyVersion, // individual values must be smaller than 65535
      string fileDescription =
          " ", // the old compiler put blank here if nothing was user-supplied
      string legalCopyright =
          " ", // the old compiler put blank here if nothing was user-supplied
      string legalTrademarks = null, string productName = null,
      string comments = null, string companyName = null) {
    ...
    var ver = new VersionResourceSerializer(
        isDll, comments, companyName, fileDescription, fileVersion,
        internalName, legalCopyright, legalTrademarks, originalFileName,
        productName, productVersion, assemblyVersion);
    ...
    resWriter.Write(
        (WORD)0x0030); // memory flags (this is what the Dev10 compiler uses)
  ...
  internal static void AppendManifestToResourceStream(Stream resStream,
    ...
    resWriter.Write(
        (WORD)((isDll)
                   ? 0x0002
                   : 0x0001)); // EDMAURER executables are named "1", DLLs "2"
    resWriter.Write((DWORD)0x00000000); // data version
  ...
  private class VersionResourceSerializer {
    ...
    internal VersionResourceSerializer(
        bool isDll, string comments, string companyName, string fileDescription,
        string fileVersion, string internalName, string legalCopyright,
        string legalTrademark, string originalFileName, string productName,
        string productVersion, Version assemblyVersion) {
      _isDll = isDll;
      ...
      _langIdAndCodePageKey = System.String.Format("{0:x4}{1:x4}", 0 /*langId*/,
    ...
    private IEnumerable<KeyValuePair<string, string>> GetVerStrings() {
      if (_commentsContents != null)
        yield return new KeyValuePair<string, string>("Comments",
                                                      _commentsContents);
      if (_companyNameContents != null)
        yield return new KeyValuePair<string, string>("CompanyName",
                                                      _companyNameContents);
      if (_fileDescriptionContents != null)
        yield return new KeyValuePair<string, string>("FileDescription",
                                                      _fileDescriptionContents);

      yield return new KeyValuePair<string, string>("FileVersion",
                                                    _fileVersionContents);

      if (_internalNameContents != null)
        yield return new KeyValuePair<string, string>("InternalName",
                                                      _internalNameContents);
      if (_legalCopyrightContents != null)
        yield return new KeyValuePair<string, string>("LegalCopyright",
                                                      _legalCopyrightContents);
      if (_legalTrademarksContents != null)
        yield return new KeyValuePair<string, string>("LegalTrademarks",
                                                      _legalTrademarksContents);
      if (_originalFileNameContents != null)
        yield return new KeyValuePair<string, string>(
            "OriginalFilename", _originalFileNameContents);
      if (_productNameContents != null)
        yield return new KeyValuePair<string, string>("ProductName",
                                                      _productNameContents);

      yield return new KeyValuePair<string, string>("ProductVersion",
                                                    _productVersionContents);

      if (_assemblyVersionContents != null)
        yield return new KeyValuePair<string, string>(
            "Assembly Version", _assemblyVersionContents.ToString());
    }
    ...
    private void WriteVSFixedFileInfo(BinaryWriter writer) {
      ...
      // The documentation says that if they're not that format the behavior is
      // undefined.
      VersionHelper.TryParse(_fileVersionContents,
                             version: out Version fileVersion);

      VersionHelper.TryParse(_productVersionContents,
                             version: out Version productVersion);

      writer.Write((DWORD)0xFEEF04BD);
      writer.Write((DWORD)0x00010000);
      writer.Write((DWORD)((uint)fileVersion.Major << 16) |
                   (uint)fileVersion.Minor);
      writer.Write((DWORD)((uint)fileVersion.Build << 16) |
                   (uint)fileVersion.Revision);
      writer.Write((DWORD)((uint)productVersion.Major << 16) |
                   (uint)productVersion.Minor);
      writer.Write((DWORD)((uint)productVersion.Build << 16) |
                   (uint)productVersion.Revision);
      writer.Write((DWORD)0x0000003F); // VS_FFI_FILEFLAGSMASK  (EDMAURER)
                                       // really? all these bits are valid?
    ...
    /// <summary>
    /// Assume that 3 WORDs preceded this string and that they began 32-bit
    /// aligned. Given the string length compute the number of bytes that should
    /// be written to end the buffer on a 32-bit boundary</summary> <param
    /// name="cb"></param> <returns></returns>
    ...
    /// <param name="cb"></param>
    /// <returns></returns>
    private static int PadToDword(int cb) { return (cb + 3) & ~3; }
    ...
    private static ushort SizeofVerString(string lpszKey, string lpszValue) {
      ...
      return checked(
          (ushort)(PadKeyLen(cbKey) + // key, 0 padded to DWORD boundary
                   cbValue +          // value
                   HDRSIZE));         // block header.
    }

    private static void
    WriteVersionString(KeyValuePair<string, string> keyValuePair,
                       BinaryWriter writer) {
      ...
      int cbVal =
          (keyValuePair.Value.Length + 1) * 2; // includes terminating NUL
    ...
    /// <summary>
    /// compute number of chars needed to end up on a 32-bit boundary assuming
    /// that three WORDS preceded this string.
    /// </summary>
    /// <param name="sz"></param>
    /// <returns></returns>
    ...
    private int GetStringsSize() {
      ...
      foreach (var verString in GetVerStrings()) {
        sum = (sum + 3) & ~3; // ensure that each String data structure starts
                              // on a 32bit boundary.
    ...
    internal int GetDataSize() {
      int sizeEXEVERRESOURCE =
          sizeof(WORD) * 3 * 5 +
          2 * sizeof(WORD) + // five headers + two words for CP and lang
          KEYBYTES(vsVersionInfoKey) + KEYBYTES(varFileInfoKey) +
          KEYBYTES(translationKey) + KEYBYTES(stringFileInfoKey) +
          KEYBYTES(_langIdAndCodePageKey) + sizeVS_FIXEDFILEINFO;
    ...
    internal void WriteVerResource(BinaryWriter writer) {
       ...
       *
       * the sub-elements of the VS_VERSIONINFO consist of a header (3 WORDS) a
       string
       * and then beginning on the next 32-bit boundary, the elements children

          struct VS_VERSIONINFO
          {
              WORD cbRootBlock;                                     // size of
       whole resource WORD cbRootValue;                                     //
       size of VS_FIXEDFILEINFO structure WORD fRootText; // root is text? WCHAR
       szRootKey[KEYSIZE("VS_VERSION_INFO")];          // Holds
       "VS_VERSION_INFO" VS_FIXEDFILEINFO vsFixed; // fixed information. WORD
       cbVarBlock;                                      //   size of VarFileInfo
       block WORD cbVarValue;                                      //   always 0
                WORD fVarText;                                        //
       VarFileInfo is text? WCHAR szVarKey[KEYSIZE("VarFileInfo")]; //   Holds
       "VarFileInfo" WORD cbTransBlock;                                    //
       size of Translation block WORD cbTransValue; //     size of Translation
       value WORD fTransText;                                      //
       Translation is text? WCHAR szTransKey[KEYSIZE("Translation")]; // Holds
       "Translation" WORD langid;                                          //
       language id WORD codepage;                                        //
       codepage id WORD cbStringBlock;                                   // size
       of StringFileInfo block WORD cbStringValue; //   always 0 WORD
       fStringText;                                     //   StringFileInfo is
       text? WCHAR szStringKey[KEYSIZE("StringFileInfo")];         //   Holds
       "StringFileInfo" WORD cbLangCpBlock;                                   //
       size of language/codepage block WORD cbLangCpValue; //     always 0 WORD
       fLangCpText;                                     //     LangCp is text?
                  WCHAR szLangCpKey[KEYSIZE("12345678")];               // Holds
       hex version of language/codepage
              // followed by strings
          };
      */
      ...
      writer.Write(
          new byte[KEYBYTES(vsVersionInfoKey) - vsVersionInfoKey.Length * 2]);
      Debug.Assert((writer.BaseStream.Position & 3) == 0);
      WriteVSFixedFileInfo(writer);
      writer.Write((WORD)(sizeof(WORD) * 2 + 2 * HDRSIZE +
                          KEYBYTES(varFileInfoKey) + KEYBYTES(translationKey)));
      writer.Write((WORD)0);
      writer.Write((WORD)1);
      writer.Write(varFileInfoKey.ToCharArray());
      writer.Write(new byte[KEYBYTES(varFileInfoKey) -
                            varFileInfoKey.Length * 2]); // padding
      Debug.Assert((writer.BaseStream.Position & 3) == 0);
      writer.Write(
          (WORD)(sizeof(WORD) * 2 + HDRSIZE + KEYBYTES(translationKey)));
      writer.Write((WORD)(sizeof(WORD) * 2));
      writer.Write((WORD)0);
      writer.Write(translationKey.ToCharArray());
      writer.Write(new byte[KEYBYTES(translationKey) -
                            translationKey.Length * 2]); // padding
      Debug.Assert((writer.BaseStream.Position & 3) == 0);
      writer.Write(
          (WORD)0); // langId; MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL)) = 0
      writer.Write((WORD)CP_WINUNICODE); // codepage; 1200 = CP_WINUNICODE
      Debug.Assert((writer.BaseStream.Position & 3) == 0);
      writer.Write((WORD)(2 * HDRSIZE + KEYBYTES(stringFileInfoKey) +
                          KEYBYTES(_langIdAndCodePageKey) + GetStringsSize()));
      writer.Write((WORD)0);
      writer.Write((WORD)1);
      writer.Write(
          stringFileInfoKey.ToCharArray()); // actually preceded by 5 WORDS so
                                            // not consistent with the
      // assumptions of KEYBYTES, but equivalent.
      writer.Write(new byte[KEYBYTES(stringFileInfoKey) -
                            stringFileInfoKey.Length * 2]); // padding.
      Debug.Assert((writer.BaseStream.Position & 3) == 0);
      writer.Write(
          (WORD)(HDRSIZE + KEYBYTES(_langIdAndCodePageKey) + GetStringsSize()));
      writer.Write((WORD)0);
      writer.Write((WORD)1);
      writer.Write(_langIdAndCodePageKey.ToCharArray());
      writer.Write(new byte[KEYBYTES(_langIdAndCodePageKey) -
                            _langIdAndCodePageKey.Length * 2]); // padding
      Debug.Assert((writer.BaseStream.Position & 3) == 0);

      Debug.Assert(writer.BaseStream.Position - debugPos ==
                   dataSize - GetStringsSize());
      debugPos = writer.BaseStream.Position;

      foreach (var entry in GetVerStrings()) {
        var writerPos = writer.BaseStream.Position;

        // write any padding necessary to align the String struct on a 32 bit
        // boundary.
  ...
  private static void AppendDefaultVersionResourceFromAstData(
      Stream resourceStream, Dictionary<Type, Ast.Attribute> attributes,
      bool isApplication) {
    var context = CompilerContext.Current;
    attributes.TryGetValue(typeof(AssemblyVersionAttribute),
                           out var versionAttr);
    var version =
        new AssemblyVersionAttribute(FirstStringArg(versionAttr) ?? "0.0.0.0")
            .Version;
    attributes.TryGetValue(typeof(AssemblyTitleAttribute), out var titleAttr);
    var filename = context.Parameters.OutputAssembly;
    attributes.TryGetValue(typeof(AssemblyInformationalVersionAttribute),
                           out var informationVersionAttr);
    attributes.TryGetValue(typeof(AssemblyCopyrightAttribute),
                           out var copytrightAttr);
    attributes.TryGetValue(typeof(AssemblyTrademarkAttribute),
                           out var trademarkAttr);
    attributes.TryGetValue(typeof(AssemblyProductAttribute),
                           out var productNameAttr);
    attributes.TryGetValue(typeof(AssemblyDescriptionAttribute),
                           out var descAttr);
    attributes.TryGetValue(typeof(AssemblyCompanyAttribute),
                           out var companyAttr);
    AppendVersionToResourceStream(
        resourceStream, !isApplication, fileVersion: version,
        originalFileName: filename, internalName: filename,
        productVersion: FirstStringArg(informationVersionAttr) ?? version,
        fileDescription: FirstStringArg(titleAttr) ??
            " ", // alink would give this a blank if nothing was supplied.
        assemblyVersion: new Version(version),
        legalCopyright: FirstStringArg(copytrightAttr) ??
            " ", // alink would give this a blank if nothing was supplied.
  ...
  private static void
  AppendDefaultVersionResource(Stream resourceStream,
                               AssemblyBuilder sourceAssembly,
                               bool isApplication) {
    var context = CompilerContext.Current;
    var versionAttr =
        sourceAssembly.GetCustomAttribute<AssemblyVersionAttribute>() ??
        new AssemblyVersionAttribute("0.0.0.0");
    var titleAttr = sourceAssembly.GetCustomAttribute<AssemblyTitleAttribute>();
    var filename = context.Parameters.OutputAssembly;
    var informationVersionAttr =
        sourceAssembly
            .GetCustomAttribute<AssemblyInformationalVersionAttribute>();
    var copytrightAttr =
        sourceAssembly.GetCustomAttribute<AssemblyCopyrightAttribute>();
    var trademarkAttr =
        sourceAssembly.GetCustomAttribute<AssemblyTrademarkAttribute>();
    var productNameAttr =
        sourceAssembly.GetCustomAttribute<AssemblyProductAttribute>();
    var descAttr =
        sourceAssembly.GetCustomAttribute<AssemblyDescriptionAttribute>();
    var companyAttr =
        sourceAssembly.GetCustomAttribute<AssemblyCompanyAttribute>();
    AppendVersionToResourceStream(
        resourceStream, !isApplication, fileVersion: versionAttr.Version,
        originalFileName: filename, internalName: filename,
        productVersion: informationVersionAttr?.InformationalVersion ??
            versionAttr.Version,
        fileDescription: titleAttr?.Title ??
            " ", // alink would give this a blank if nothing was supplied.
        assemblyVersion: new Version(versionAttr.Version),
        legalCopyright: copytrightAttr?.Copyright ??
            " ", // alink would give this a blank if nothing was supplied.
        legalTrademarks: trademarkAttr?.Trademark,
        productName: productNameAttr?.Product, comments: descAttr?.Description,
...
<assembly xmlns = ""urn:schemas-microsoft-com:asm.v1"" manifestVersion=""1.0"">
  ...
  /// <summary>
  /// Create a stream filled with default win32 resources, and return its
  /// contents as a byte buffer.
  /// </summary>
  internal static byte[] CreateDefaultWin32Resources(
      bool noManifest, bool isApplication, Stream manifestContents,
      Stream iconInIcoFormat, AssemblyBuilder sourceAssembly) {
    return DoCreateDefaultWin32Resources(
        noManifest, isApplication, manifestContents, iconInIcoFormat,
        (stream, isApp) =>
            AppendDefaultVersionResource(stream, sourceAssembly, isApp));
  }

  internal static byte[] CreateDefaultWin32Resources(bool noManifest,
                                                     bool isApplication,
                                                     Stream manifestContents,
                                                     Stream iconInIcoFormat,
                                                     CompileUnit compilation) {
    var asmAttributes =
        compilation.Modules.SelectMany(m => m.AssemblyAttributes)
            .Where(a => a.Entity is ExternalType)
            .ToDictionary(a => ((ExternalType)a.Entity).ActualType);
    return DoCreateDefaultWin32Resources(
        noManifest, isApplication, manifestContents, iconInIcoFormat,
        (stream, isApp) => AppendDefaultVersionResourceFromAstData(
            stream, asmAttributes, isApp));
  }
  ...
  /// contents as a byte buffer.
  /// </summary>
  private static byte[] DoCreateDefaultWin32Resources(
      bool noManifest, bool isApplication, Stream manifestContents,
      Stream iconInIcoFormat, Action<Stream, bool> writeVersionResource) {
    // Win32 resource encodings use a lot of 16bit values. Do all of the math
    // checked with the expectation that integer types are well-chosen with size
    // in mind.
    checked {
      ...
      if (!noManifest) {
        ...
        if (manifestContents != null) {
          AppendManifestToResourceStream(result, manifestContents,
                                         !isApplication);
        }
      }
